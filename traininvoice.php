...
<head lang="en">
	...
	<body>
		...
		<form class="form-horizontal"  method="POST">
				...
				<div class="form-group">
					<label class="control-label col-sm-5" for="class">Train Name:</label>
					<div class="col-sm-6">
						<?php
							echo '<label class="control-label col-sm-5" for="class">'.$train.'</label>';
						?>
				...
				<div class="form-group">
					...
					<div class="col-sm-6">
						<?php
							if('yes'==$food){
								include ('C:\xampp\htdocs\tour\database\config.php');
								$sql = "SELECT * FROM ADD_TRAIN WHERE TRAINNAME='$train' AND AFROM='$from' AND ATO='$to'";
								$result = $con->query($sql);
								if('AC'==$class){
									
									if ($result->num_rows > 0) {
										while($row = $result->fetch_assoc()) {
											$price=$noofpassenger * ($row['AC'] + $row['FOOD']);
											//echo '<label class="control-label col-sm-5" for="class">'.$price.'</label>';
										}
									}
								}
								elseif('SLEEPER'==$class){
									/*include ('C:\xampp\htdocs\tour\database\config.php');
									$sql = "SELECT * FROM ADD_BUS WHERE BUSNAME='$bus'";
									$result = $con->query($sql);*/
									if ($result->num_rows > 0) {
										while($row = $result->fetch_assoc()) {
											$price=$noofpassenger * ($row['SLEEPER'] + $row['FOOD']);
											//echo '<label class="control-label col-sm-5" for="class">'.$price.'</label>';
										}
									}
								}
								elseif('SITTING'==$class){
									/*include ('C:\xampp\htdocs\tour\database\config.php');
									$sql = "SELECT * FROM ADD_BUS WHERE BUSNAME='$bus'";
									$result = $con->query($sql);*/
									if ($result->num_rows > 0) {
										while($row = $result->fetch_assoc()) {
											$price=$noofpassenger * ($row['SITTING'] + $row['FOOD']);
											//echo '<label class="control-label col-sm-5" for="class">'.$price.'</label>';
										}
									}
								}
							...
							elseif('no'==$food){
								...
								elseif('SLEEPER'==$class){
									...
									if ($result->num_rows > 0) {
										while($row = $result->fetch_assoc()) {
											$price=$noofpassenger * $row['SLEEPER'];
											//echo '<label class="control-label col-sm-5" for="class">'.$price.'</label>';
										}
									}
								}
								elseif('SITTING'==$class){
									...
									if ($result->num_rows > 0) {
										while($row = $result->fetch_assoc()) {
											$price=$noofpassenger * $row['SITTING'];
											//echo '<label class="control-label col-sm-5" for="class">'.$price.'</label>';
										}
									}
								}
