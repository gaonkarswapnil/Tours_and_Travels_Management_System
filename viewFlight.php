...
<head lang="en">
	...
	<body>
		...
		<form class="form-horizontal"  method="POST">
			<div class="form-group">
					<div class="media-left media-middle">
						...
						$sql = "SELECT * FROM ADD_FLIGHT WHERE AFROM='$from' AND ATO='$to' AND FLIGHTNAME='$flight'";
						$result = $con->query($sql);
						
						if ($result->num_rows > 0) {
							while($row = $result->fetch_assoc()) {
								$imageURL = 'images/'.$row["IMAGE"];
								//$_SESSION["session_name"]=$row["FLIGHTNAME"];
								$_SESSION["session_deptime"]=$row["TAKEOFTIME"];
								$_SESSION["session_artime"]=$row["ARRIVALTIME"];
								
								echo '<table align="center">';
								echo '<tr><td rowspan="10"><img src='.$imageURL.' alt="" /></td>';
								echo '<td colspan="2" align="center" name="FLIGHTNAME"><h3>'.$row["FLIGHTNAME"].'</h3></td></tr>';
								echo '<tr><td align="center"><b>From </b></td><td><input type="text" class="no-outline" value='.$row["AFROM"].' readonly></td></tr>';
								echo '<tr><td align="center"><b>Time </b></td><td><input type="text" class="no-outline" value='.$row["TAKEOFTIME"].' readonly></td></tr>';
								...
								echo '<tr><td align="center"><b>Economy Class </b></td><td><input type="textbox" class="no-outline" value='.$row["ECONOMY"].' readonly></td></tr>';
								echo '<tr><td align="center"><b>Business Class </b></td><td><input type="textbox" class="no-outline" value='.$row["BUSINESS"].' readonly></td></tr>';
								echo '<tr><td align="center"><b>First Class </b></td><td><input type="textbox" class="no-outline" value='.$row["FIRSTCLASS"].' readonly></td></tr>';
					...
					<!--<div class="media-body" align="center">
						...
						$sql = "SELECT * FROM ADD_FLIGHT WHERE AFROM='$from' AND ATO='$to'";
						$result = $con->query($sql);
						if ($result->num_rows > 0) {
							while($row = $result->fetch_assoc()) {
								echo '<h3>'.$row["FLIGHTNAME"].'</h3>';
								echo '<table>';
								echo '<tr><td><b>From </b></td><td><input type="text" class="no-outline" value='.$row["AFROM"].' readonly></td></tr>';
								echo '<tr><td><b>Time </b></td><td><input type="text" value='.$row["TAKEOFTIME"].' readonly></td></tr>';
								echo '<tr><td><b>To </b></td><td><input type="text" value='.$row["ATO"].' readonly></td></tr>';
								echo '<tr><td><b>Time </b></td><td><input type="textbox" value='.$row["ARRIVALTIME"].' readonly></td></tr>';
								echo '<tr><td><b>Economy Class </b></td><td><input type="textbox" value='.$row["ECONOMY"].' readonly></td></tr>';
								echo '<tr><td><b>Business Class </b></td><td><input type="textbox" value='.$row["BUSINESS"].' readonly></td></tr>';
								echo '<tr><td><b>First Class </b></td><td><input type="textbox" value='.$row["FIRSTCLASS"].' readonly></td></tr>';
								echo '<tr><td><b>Additional charges for food &nbsp;&nbsp;</b></td><td><input type="textbox" value='.$row["FOOD"].' readonly></td></tr>';
								echo '</table>';
								echo '<button type="submit" class="btn btn-primary btn-md" id="checkout" name="checkout">Book Now</button>';
		...
		<footer class="text-center" id="footerID">
			<a href="viewFlight.php" data-toggle="tooltip" title="TO TOP">
