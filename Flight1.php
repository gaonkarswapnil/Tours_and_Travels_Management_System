<?php
	...
	if (isset($_POST['search'])) {
		...
		$_SESSION["session_flight"] = $flight;
...
<head lang="en">
	...
	<body>
		...
		<form class="form-horizontal"  method="POST">
				<div class="form-group">
					...
					<?php
						...
						$sql = "SELECT DISTINCT(AFROM) FROM ADD_FLIGHT ORDER BY AFROM";
				...
				<div class="form-group">
					...
					<?php
						...
						$sql = "SELECT DISTINCT(ATO) FROM ADD_FLIGHT ORDER BY ATO";
						$result = $con->query($sql);
				...
				<div class="form-group">
						<label class="control-label col-sm-4" for="to">Flight Name:</label>
						<div class="col-sm-6">	
							<?php
								include ('C:\xampp\htdocs\tour\database\config.php');
								if ($con->connect_error) {
								...
								$sql = "SELECT DISTINCT(FLIGHTNAME) FROM ADD_FLIGHT ORDER BY FLIGHTNAME";
								$result = $con->query($sql);

								if ($result->num_rows > 0) {
									echo '<select class="form-control" name="flight">';
										echo '<option value="" disabled selected>Choose your option</option>';
										while($row = $result->fetch_assoc()) {
											echo '<option>'.$row["FLIGHTNAME"].'</option>';
										}
										echo "</select>";
								}else {
									echo "0 results";
		...
		<footer class="text-center" id="footerID">
			<a href="Flight1.php" data-toggle="tooltip" title="TO TOP">
