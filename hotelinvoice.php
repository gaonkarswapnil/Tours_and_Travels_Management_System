...
<head lang="en">
	...
	<body>
		...
		<form class="form-horizontal"  method="POST">
				...
				<div class="form-group">
					<label class="control-label col-sm-5" for="class">Hotel Name:</label>
					<div class="col-sm-6">
						<?php
							echo '<label class="control-label col-sm-5" for="class">'.$hotel.'</label>';
							/*include ('C:\xampp\htdocs\tour\database\config.php');
							if ($con->connect_error) {
								die("Connection failed: " . $con->connect_error);
							}
							$sql = "SELECT * FROM HOTELBOOK WHERE CUSTOMER='$email'";
							if ($result->num_rows > 0) {
								while($row = $result->fetch_assoc()) {
									echo '<label class="control-label col-sm-" for="class">'.$row['HOTELNAME'].'</label>';
								}
							}else {
								echo "No data";
							}
				...
				<div class="form-group">
					<label class="control-label col-sm-5" for="class">Destination:</label>
					<div class="col-sm-6">
						<?php
							echo '<label class="control-label col-sm-5" for="class">'.$option.'</label>';
						?>
					</div>
				</div>
				
				<div class="form-group">
					<label class="control-label col-sm-5" for="class">Address:</label>
					<div class="col-sm-6">
						<?php
							echo '<label class="control-label col-sm-5" for="class">'.$address.'</label>';
						?>
					</div>
				</div>
				
				<div class="form-group">
					<label class="control-label col-sm-5" for="class">Check-In Date:</label>
					<div class="col-sm-6">
						<?php
							echo '<label class="control-label col-sm-5" for="class">'.$cidot.'</label>';
						?>
					</div>
				</div>
				
				<div class="form-group">
					<label class="control-label col-sm-5" for="class">Check-out Date:</label>
					<div class="col-sm-6">
						<?php
							echo '<label class="control-label col-sm-5" for="class">'.$codot.'</label>';
						?>
					</div>
				</div>
				...
				<div class="form-group">
					<label class="control-label col-sm-5" for="class">Room:</label>
					<div class="col-sm-6">
						<?php
							echo '<label class="control-label col-sm-5" for="class">'.$room.'</label>';
						?>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-6">
						<?php
							if('yes'==$food){
								include ('C:\xampp\htdocs\tour\database\config.php');
								$sql = "SELECT * FROM ADD_HOTEL WHERE HOTELNAME='$hotel' AND ASTATE='$option'";
								$result = $con->query($sql);
								if('AC'==$room){
									
									if ($result->num_rows > 0) {
										while($row = $result->fetch_assoc()) {
											$price=($row['AC'] + $row['FOOD']);
											//echo '<label class="control-label col-sm-5" for="class">'.$price.'</label>';
										}
									}
								}
								elseif('STANDARD'==$room){
									if ($result->num_rows > 0) {
										while($row = $result->fetch_assoc()) {
											$price=($row['STANDARD'] + $row['FOOD']);
											//echo '<label class="control-label col-sm-5" for="class">'.$price.'</label>';
										}
									}
								}
