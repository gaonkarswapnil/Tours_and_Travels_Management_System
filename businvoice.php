...
<head lang="en">
	...
	<body>
		...
		<form class="form-horizontal"  method="POST">
				...
				<div class="form-group">
					<label class="control-label col-sm-5" for="class">Bus Name:</label>
					<div class="col-sm-6">
						<?php
							echo '<label class="control-label col-sm-5" for="class">'.$bus.'</label>';
						?>
				...
				<div class="form-group">
					...
					<div class="col-sm-6">
						<?php
							if('yes'==$food){
								include ('C:\xampp\htdocs\tour\database\config.php');
								$sql = "SELECT * FROM ADD_BUS WHERE BUSNAME='$bus' AND AFROM='$from' AND ATO='$to'";
								$result = $con->query($sql);
								if('AC'==$class){
								...
								elseif('DELUXE'==$class){
									...
									if ($result->num_rows > 0) {
										while($row = $result->fetch_assoc()) {
											$price=$noofpassenger * ($row['DELUXE'] + $row['FOOD']);
											//echo '<label class="control-label col-sm-5" for="class">'.$price.'</label>';
										}
									}
								}
							...
							elseif('no'==$food){
								...
								elseif('DELUXE'==$class){
									...
									if ($result->num_rows > 0) {
										while($row = $result->fetch_assoc()) {
											$price=$noofpassenger * $row['DELUXE'];
											//echo '<label class="control-label col-sm-5" for="class">'.$price.'</label>';
										}
									}
								}
